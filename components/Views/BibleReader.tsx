...
const getColorClasses = (color: string, isSelected: boolean = false) => {
  const colors: Record<string, { bg: string; text: string; border: string; hover: string; light: string }> = {
    ...
    rose: {
      bg: isSelected ? 'bg-rose-100' : 'bg-white',
      text: 'text-rose-900',
...
export const BibleReader: React.FC = () => {
  ...
  const groupedBooks = useMemo(() => {
    const grouped: Record<string, Record<string, { book: string; index: number; chapters: any[] }[]>> = {
      'Senasis Testamentas': {},
      'Naujasis Testamentas': {}
    };
  ...
  const filteredBooks = useMemo(() => {
    ...
    const filtered: Record<string, Record<string, { book: string; index: number; chapters: any[] }[]>> = {
      'Senasis Testamentas': {},
      'Naujasis Testamentas': {}
    };
  ...
  const handleBookSelect = (bookIndex: number) => {
    ...
    setSelectedChapter(0); // Auto-select first chapter
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-6 border-b border-stone-200 bg-white">
        <button
          ...
          className="flex items-center gap-2 text-stone-500 hover:text-red-900 transition-colors mb-4"
        >
          <ArrowLeft size={16} />
          <span className="text-sm font-medium">Grįžti prie knygų sąrašo</span>
        </button>
        <h2 className="font-cinzel font-bold text-2xl text-stone-900">{bookName}</h2>
        <p className="text-stone-500 text-sm mt-1">Pasirinkite skyrių</p>
      </div>
  ...
  const BookGrid = ({ books, selectedBook, onSelect, color }: {
    books: { book: string; index: number; chapters: any[] }[];
        ...
        {books.map((bookData) => (
          <button
            ...
            className={`
              ...
              text-xs font-medium transition-all duration-200 transform hover:scale-105 p-2
              ${selectedBook === bookData.index
                ? `${colors.bg} ${colors.text} shadow-lg scale-105 ring-1 ${colors.border}`
                : 'bg-stone-100 text-stone-700 hover:bg-stone-200 hover:shadow-md'
              }
            `}
            title={bookData.book}
          >
            <span className="text-center leading-tight line-clamp-3">
              {bookData.book}
            </span>
          </button>
        ))}
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-64px)] bg-[#fdfcf8]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-red-900 mx-auto mb-4" />
          <p className="text-stone-600 font-serif">Kraunama Biblija...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-[#f4f1ea]">
      {/* Sidebar - Book Navigation */}
      <div className={`
        fixed inset-y-0 left-0 z-20 w-80 bg-[#f8f6f3] border-r border-[#e5e0d8] transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 flex flex-col
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>  <div className="p-4 border-b border-stone-100 bg-gradient-to-r from-red-900 to-red-800">
          <h2 className="font-cinzel font-bold text-amber-50 flex items-center gap-2 mb-4">
        ...
        {/* Info Banner */}
        <div className="mx-4 mt-4 bg-amber-50 border border-amber-200 rounded-lg p-3 flex gap-2">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-3">
          {Object.entries(filteredBooks).map(([testament, categories]) => {
            ...
            return (
              <div key={testament} className="border border-stone-200 rounded-xl overflow-hidden bg-stone-50/50">
                {/* Testament Header */}
                <button
                ...
                {isTestamentExpanded && (
                  <div className="p-2 space-y-2">
                    {Object.entries(categories).map(([category, books]) => {
                      ...
                      return (
                        <div key={category} className="bg-white rounded-lg border border-stone-100 overflow-hidden">
                          {/* Category Header */}
                          <button
                            ...
                            className={`
                              ...
                              transition-all duration-200
                              ${getColorClasses(color).light} ${getColorClasses(color).text}
                              hover:brightness-95
                            `}
                          >
                            <span className="flex items-center gap-2">
      ...
      <main
        ref={mainContentRef}
        className={`flex-1 overflow-y-auto relative bg-[url('https://www.transparenttextures.com/patterns/paper-fibers.png')] transition-all duration-300 ${isChatOpen ? 'lg:mr-[0px]' : ''}`}
        ...
        {selectedBook === null && (
          <div className="h-full flex items-center justify-center p-8">
            <div className="text-center max-w-md">
              ...
              <h2 className="font-cinzel font-bold text-2xl text-stone-900 mb-3">
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <header className="mb-8 text-center">
                ...
                <div className="flex items-center justify-center gap-2 mb-4 text-sm flex-wrap">
                  ...
                  <ChevronRight size={14} className="text-stone-400" />
                  <span className="text-red-900 font-medium">Skyrius {selectedChapter + 1}</span>
                </div>

                <h1 className="font-cinzel font-bold text-2xl md:text-3xl lg:text-4xl text-stone-900">{currentChapter.title}</h1>
                <div className="h-px w-12 bg-red-900 mx-auto mt-4" />
              </header>

              <article
                className="font-serif text-lg md:text-xl text-stone-800 leading-loose text-justify whitespace-pre-wrap selection:bg-amber-200 selection:text-amber-950 cursor-text"
              >
                {parsedContent ? (
      ...
      {/* Chat Sidebar */}
      {isChatOpen && (
        <aside className="fixed inset-y-0 right-0 w-full sm:w-[450px] bg-white border-l border-stone-200 shadow-2xl z-[100] flex flex-col animate-in slide-in-from-right duration-300">
        ...
        isMobile && (
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 shadow-lg z-50 lg:hidden">
            <div className="flex items-center justify-around p-2">
              <button
                onClick={() => {
                  handleBackToBookSelection();
                  setIsChatOpen(true);
                }}
                className="flex flex-col items-center gap-1 p-2 text-stone-600 hover:text-red-900 transition-colors"
              >
                <Book size={20} />
                <span className="text-[10px] font-medium">Knyga</span>
              </button>
              {selectedBook !== null && (
                <button
                  onClick={handleBackToChapterSelection}
                ...
                >
                  <Grid3X3 size={20} />
                  <span className="text-[10px] font-medium">Skyriai</span>
                </button>
              )}
              <button
                onClick={() => setIsChatOpen(!isChatOpen)}
                className={`flex flex-col items-center gap-1 p-2 transition-colors ${isChatOpen ? 'text-red-900' : 'text-stone-600'}`}
              >
