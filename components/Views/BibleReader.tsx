...
import { SharedScripturePayload } from '../../types';
...
interface BibleReaderProps {
  onShareToGroup?: (payload: SharedScripturePayload) => void;
}
...
export const BibleReader: React.FC<BibleReaderProps> = ({ onShareToGroup }) => {
  ...
  const handleShareSelectedVerses = () => {
    if (!onShareToGroup || !parsedContent?.verses || selectedVerses.length === 0 || !currentBook || selectedChapter === null) {
      return;
    }

    const verseNumbers = [...selectedVerses].sort((a, b) => a - b);
    const selectedText = verseNumbers.map(vNum => {
      const v = parsedContent.verses.find((cv: any) => cv.number === vNum);
      return v ? `${v.number}. ${v.content}` : '';
    }).join('\n');

    const reference = `${currentBook.book} ${selectedChapter + 1}:${verseNumbers.join(',')}`;
                ...
                {parsedContent ? (
                  <div className={`leading-relaxed text-lg space-y-4 ${isDark ? 'text-slate-300' : 'text-stone-800'}`}>
                    ...
                    {selectedVerses.length > 0 && (
                      <div className={`fixed top-0 left-0 right-0 z-[200] flex items-center justify-end gap-2 px-3 py-2.5 backdrop-blur-xl border-b shadow-lg ${isDark ? 'bg-slate-900/95 border-amber-500/30' : 'bg-white/95 border-amber-200'}`}>
                        <div className="flex gap-2">
                          ...
                          {onShareToGroup && (
                            <button onClick={handleShareSelectedVerses} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold ${isDark ? 'bg-emerald-900/70 text-emerald-200 hover:bg-emerald-800' : 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200'}`}>
                              Dalintis grupėje
                            </button>
                          )}
