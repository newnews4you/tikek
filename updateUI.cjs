                      ...
                      <div key={post.id} className={\`flex \${isOwnPost ? 'justify-end' : 'justify-start'}\`}>
                        <div className={\`flex flex-col max-w-[85%] md:max-w-[70%] \${isOwnPost ? 'items-end' : 'items-start'}\`}>
                          ...
                          <div className={\`relative group px-4 py-3 rounded-2xl shadow-sm text-sm whitespace-pre-line \${
                            ...
                            {post.scripture_text && (
                              <div className={\`mb-2 pb-2 border-b \${isOwnPost ? 'border-amber-50/20' : isDark ? 'border-slate-700' : 'border-stone-200'}\`}>
                             ...
                             {REACTIONS.map(emoji => {
                               ...
                               return (
                                  <button
                                    ...
                                    className={\`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border shadow-sm \${
                ...
                {showBibleAttachment && (
                  <div className={\`mb-3 p-3 rounded-xl border \${isDark ? 'bg-[#0f172a] border-slate-700' : 'bg-stone-50 border-stone-200'} animate-in slide-in-from-bottom-2\`}>
                     ...
                     <textarea
                        ...
                        className={\`w-full text-sm rounded-lg border px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-red-900/50 resize-none \${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-stone-300'}\`}
                     />
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className={\`relative w-full max-w-sm rounded-2xl p-6 shadow-2xl animate-in zoom-in-95 \${isDark ? 'bg-slate-900 border border-slate-700' : 'bg-white'}\`}>
             ...
             <form onSubmit={e => { submitCreateGroup(e); if(isCreateNameValid) setShowCreateGroup(false); }} className="space-y-3">
                <input value={createName} onChange={e => setCreateName(e.target.value)} placeholder="Grupės pavadinimas" className={\`w-full rounded-xl border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-900/50 \${isDark ? 'bg-slate-950 border-slate-700' : 'bg-stone-50 border-stone-300'}\`} autoFocus />
                <textarea value={createDescription} onChange={e => setCreateDescription(e.target.value)} placeholder="Aprašymas (nebūtina)" rows={3} className={\`w-full rounded-xl border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-900/50 resize-none \${isDark ? 'bg-slate-950 border-slate-700' : 'bg-stone-50 border-stone-300'}\`} />
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className={\`relative w-full max-w-sm rounded-2xl p-6 shadow-2xl animate-in zoom-in-95 \${isDark ? 'bg-slate-900 border border-slate-700' : 'bg-white'}\`}>
             ...
             <form onSubmit={e => { submitJoin(e); if(joinCode.trim()) setShowJoinGroup(false); }} className="space-y-3">
                ...
                <textarea value={joinMessage} onChange={e => setJoinMessage(e.target.value)} placeholder="Trumpa žinutė grupei (nebūtina)" rows={2} className={\`w-full rounded-xl border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-900/50 resize-none \${isDark ? 'bg-slate-950 border-slate-700' : 'bg-stone-50 border-stone-300'}\`} />
                <div className="flex gap-2 pt-2">
          ...
          <aside className={\`relative w-full max-w-md h-full flex flex-col shadow-2xl animate-in slide-in-from-right-8 \${isDark ? 'bg-slate-900 border-l border-slate-700' : 'bg-white border-l border-stone-300'}\`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black/5 dark:bg-black/20">
              <div className="p-3 rounded-xl bg-white dark:bg-slate-800 border dark:border-slate-700 shadow-sm opacity-80 text-sm italic whitespace-pre-line relative">
                ...
                {drawerPost.comment || "Nėra pradinės žinutės komentaro."}
              </div>
